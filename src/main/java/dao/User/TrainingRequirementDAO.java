...
public class TrainingRequirementDAO {

    public TrainingRequirement findActiveByUserId(int userId) throws Exception {
        String sql = "SELECT TOP 1 requirementId, userId, goal, availableTime, preferredDays, isCompleted, createdAt " +
                     "FROM TrainingRequirement WHERE userId = ? AND isCompleted = 0 " +
        ...
        try (Connection con = DBContext.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    return mapResultSetToTrainingRequirement(rs);
                }
            }
        }
        return null;
    ...
    public TrainingRequirement findLatestByUserId(int userId) throws Exception {
        String sql = "SELECT TOP 1 requirementId, userId, goal, availableTime, preferredDays, isCompleted, createdAt " +
                     "FROM TrainingRequirement WHERE userId = ? " +
                     "ORDER BY createdAt DESC, requirementId DESC";
    ...
    public int insertNew(TrainingRequirement tr) throws Exception {
        String sql = "INSERT INTO TrainingRequirement(userId, goal, availableTime, preferredDays, isCompleted) " +
                     "VALUES(?, ?, ?, ?, 0)";
    ...
    public TrainingRequirement getTrainingRequirementByUserId(int userId) {
        ...
        try (Connection con = DBContext.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    return new TrainingRequirement(
                        rs.getInt("requirementId"), 
                        rs.getInt("userId"), 
                        rs.getString("goal"), 
                        rs.getString("availableTime"), 
                        rs.getString("preferredDays"), 
    ...
    private TrainingRequirement mapResultSetToTrainingRequirement(ResultSet rs) throws SQLException {
