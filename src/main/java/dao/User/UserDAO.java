...
public class UserDAO {
    ...
    // Hàm mapping dữ liệu từ ResultSet sang Object
    ...
    public int insert(User u) throws Exception {
        String sql = "INSERT INTO dbo.[User] (accountId, name, age, gender, height, weight, fitnessLevel) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (Connection con = DBContext.getConnection();
             PreparedStatement ps = con.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ps.setInt(1, u.getAccountId());
            ps.setString(2, u.getName());
            if (u.getAge() == null) ps.setNull(3, Types.INTEGER); else ps.setInt(3, u.getAge());
            ps.setString(4, u.getGender());
            if (u.getHeight() == null) ps.setNull(5, Types.DOUBLE); else ps.setDouble(5, u.getHeight());
            if (u.getWeight() == null) ps.setNull(6, Types.DOUBLE); else ps.setDouble(6, u.getWeight());
    ...
    public boolean updateByAccountId(User u) throws Exception {
        String sql = "UPDATE dbo.[User] SET name=?, age=?, gender=?, height=?, weight=?, fitnessLevel=? WHERE accountId=?";
        try (Connection con = DBContext.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1, u.getName());
            if (u.getAge() == null) ps.setNull(2, Types.INTEGER); else ps.setInt(2, u.getAge());
            ps.setString(3, u.getGender());
            if (u.getHeight() == null) ps.setNull(4, Types.DOUBLE); else ps.setDouble(4, u.getHeight());
            if (u.getWeight() == null) ps.setNull(5, Types.DOUBLE); else ps.setDouble(5, u.getWeight());
    ...
    public void upsertByAccountId(User u) throws Exception {
        if (findByAccountId(u.getAccountId()) == null) {
            insert(u);
        } else {
            updateByAccountId(u);
        }
